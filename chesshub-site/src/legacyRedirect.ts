...
const target: Record<LegacySection, { hub: string; type: string }> = {
  work: { hub: '/journal.html', type: 'work' },
  study: { hub: '/journal.html', type: 'study' },
  figure: { hub: '/life.html', type: 'figure' },
  travel: { hub: '/life.html', type: 'travel' },
  note: { hub: '/life.html', type: 'note' },
}
...
export function redirectLegacyVisitor(section: LegacySection) {
  ...
  if (id) {
    next.searchParams.set('type', config.type)
    next.searchParams.set('id', id)
  } else {
    next.searchParams.set('filter', config.type)
  }
