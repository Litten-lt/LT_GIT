...
import { GobangState, INITIAL_GOBANG_STATE } from '../types';
...
export function createGobangReducer() {
  return function gameReducer(state: GobangState, action: any): GobangState {
    switch (action.type) {
      case 'MOVE':
      case 'AI_MOVE': {
      ...
      case 'UNDO': {
        const moveHistoryLength = state.moveHistory.length;
        if (moveHistoryLength === 0) return state;

        const undoCount = 2;
        if (moveHistoryLength < undoCount) return state;
